...
/**
 * This Class Contains All Method Related To IcmsFeaturedArticlesFragment.
 * 
 * @author tasol
 * 
 */
public class IcmsFeaturedArticlesFragment extends SmartFragment implements IcmsTagHolder {
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				if ((firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 0) {
					if (!articlesDataProvider.isCalling() && articlesDataProvider.hasNextPage()) {
						listFooterVisible();
						articlesDataProvider.getFeaturedArticles(new WebCallListenerWithCacheInfo() {
							...
							public void onCallComplete(final int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2, int pageNo,
									int pageLimit, boolean isFromCaching) {
								listFooterInvisible();
								if (responseCode == 200) {
									if (data1 != null && data1.size() > 0) {
										prepareList(data1, true, isFromCaching, pageNo, pageLimit);
									}
								}
	...
	/**
	 * This method used to preapre article ids.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * This method used to prepare list for featured article.
	...
	/**
	 * List adapter for featured article.
	...
	public SmartListAdapterWithHolder getListAdapter(ArrayList<SmartListItem> listData) {
		...
		SmartListAdapterWithHolder adapterWithHolder = new SmartListAdapterWithHolder(getActivity(), R.layout.icms_featured_article_listitem, listData, new ItemView() {
			...
			public View setItemView(int position, View v, SmartListItem item, ViewHolder holder) {
				...
				androidAQuery.id(holder.icmsImageThumb).image(value.get(IMAGE), true, true, ((SmartActivity) getActivity()).getDeviceWidth(), R.drawable.icms_article_default);
