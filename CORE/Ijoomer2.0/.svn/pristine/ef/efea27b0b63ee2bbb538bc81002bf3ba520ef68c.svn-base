			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
				if ((firstVisibleItem + visibleItemCount) >= totalItemCount && totalItemCount > 0) {
					...
					if (!articlesDataProvider.isCalling() && articlesDataProvider.hasNextPage()) {
						...
						articlesDataProvider.getArchivedArticles(new WebCallListener() {
...
//					setScreenCaption(article_extra.get(TITLE));
					loadNew(IcmsArticleDetailActivity.class, IcmsArchivedArticlesActivity.this, false, "IN_ARTICLE_INDEX", arg2, "IN_ARTICLE_TITLE", article_extra.get(TITLE),
		...
		articlesDataProvider.getArchivedArticles(new WebCallListener() {
			...
			public void onCallComplete(final int responseCode, String errorMessage, ArrayList<HashMap<String, String>> data1, Object data2) {
				if (responseCode == 200) {
					...
					IjoomerUtilities.getCustomOkDialog(getString(R.string.archived_article),
